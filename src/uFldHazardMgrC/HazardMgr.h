...
class HazardMgr : public AppCastingMOOSApp
{
 ...
 protected: // Registration, Configuration, Mail handling utils
   ...
   void handleMailMissionParams(std::string);
	 void handleMailCollReport(std::string);
	 void pointPath();
	 void postFinalReport();
	 void mergeReport(std::string);
	 void mergePoint(std::string);
	 void gotCollMail();
	 void postRePost();
 ...
 private: // Configuration variables
   ...
   double      m_pd_desired;
	 double      m_post_time;
	 double      m_wait_time;
	 std::string m_final_report;
	 std::string m_restart_point;
	 std::string m_restart_merge;
	 std::string m_restart_post;
	 std::string m_restart_final;
	 std::string m_restart_sep;
   std::string m_report_name;
	 std::string m_check_twice;
	 std::string m_point_path;
