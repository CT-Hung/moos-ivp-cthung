...
for ARGI; do
    ...
    elif [ "${ARGI:0:12}" = "--region_ew=" ] ; then
        ASSIGNED_REGION_EW="${ARGI#--region_ew=*}"

    elif [ "${ARGI:0:12}" = "--region_ns=" ] ; then
        ASSIGNED_REGION_NS="${ARGI#--region_ns=*}"

    elif [ "${ARGI:0:14}" = "--swath_width=" ] ; then
        SWATH_WIDTH="${ARGI#--swath_width=*}"
...
#-------------------------------------------------------
VNAME1="cthung"      # The first   vehicle community
VNAME2="cail"    # The second  vehicle community
START_POS="0,0"  
...
nsplug meta_vehicle.moos targ_$VNAME.moos -f WARP=$TIME_WARP \
    ...
    SHORE_LISTEN=$SHORE_LISTEN         SWATH_WIDTH=$SWATH_WIDTH \
    ASSIGNED_REGION_NS=$ASSIGNED_REGION_NS  ASSIGNED_REGION_EW=$ASSIGNED_REGION_EW
